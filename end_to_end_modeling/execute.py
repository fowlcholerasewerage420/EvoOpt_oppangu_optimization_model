...
def compile_script(script_content, timeout=300, script_id=0):
    print(f"\n[脚本 {script_id}] 开始执行调试...")
    ...
    # 保存原始脚本以供检查
    debug_file = os.path.join(target_dir, f"debug_script_{script_id}.py")
    with open(debug_file, 'w', encoding='utf-8') as f:
        f.write(script_content)
    print(f"[脚本 {script_id}] 脚本已保存到: {debug_file}")
    ...
    try:
        # 使用当前 Python 解释器
        cmd = [sys.executable, tmp_file_name]
        print(f"[脚本 {script_id}] 执行命令: {' '.join(cmd)}")
        ...
        print(f"[脚本 {script_id}] 返回码: {process.returncode}")
        print(f"[脚本 {script_id}] 标准输出长度: {len(process.stdout)}")
        print(f"[脚本 {script_id}] 标准错误长度: {len(process.stderr)}")

        if process.stdout:
            print(f"[脚本 {script_id}] 标准输出前500字符:")
            print("-" * 50)
            print(process.stdout[:500])
            if len(process.stdout) > 500:
                print(f"... (还有 {len(process.stdout) - 500} 字符)")
            print("-" * 50)

        if process.stderr:
            print(f"[脚本 {script_id}] 标准错误:")
            print("-" * 50)
            print(process.stderr)
            print("-" * 50)

        # 优先使用 stderr（因为返回码为1通常意味着错误在 stderr 中）
        execution_result = process.stderr if process.stderr else process.stdout

        # 检查是否是 COPT 错误
        if "Cardinal Optimizer" in execution_result:
            print(f"[脚本 {script_id}] 检测到 COPT 输出")
        ...
        if execution_best_solution_start_pos != -1:
            execution_best_solution = execution_result[execution_best_solution_start_pos:].replace(
                "Just print the best solution:", "").strip()
            ...
            print(f"[脚本 {script_id}] ✓ 找到最优解: {execution_best_solution}")
        else:
            if "No Best Solution" in execution_result:
                ...
                print(f"[脚本 {script_id}] ✓ 执行成功但无最优解")
            else:
                execution_best_solution = None
                if process.returncode == 0:
                    execution_state = "Execution Successful but Out of Expectation"
                    print(f"[脚本 {script_id}] ⚠ 执行成功但输出不符合预期")
                else:
                    execution_state = f"Execution Failed with return code {process.returncode}"
                    print(f"[脚本 {script_id}] ✗ 执行失败，返回码: {process.returncode}")

    except subprocess.TimeoutExpired as e:
        execution_result = e.stdout if e.stdout else "Timeout"
        execution_best_solution = None
        execution_state = "Execution Failed: Timeout"
        print(f"[脚本 {script_id}] ✗ 执行超时")
    except Exception as e:
        execution_result = str(e)
        execution_best_solution = None
        execution_state = f"Execution Failed: {type(e).__name__}"
        print(f"[脚本 {script_id}] ✗ 执行异常: {e}")
    finally:
        try:
            os.remove(tmp_file_name)
            print(f"[脚本 {script_id}] 已清理临时文件")
        except:
            pass

    execution_output = {
        "execution_result": execution_result[:1000] if execution_result else "",  # 限制长度
...
def main(args):
    # Check version
    process = subprocess.run(['python', '--version'], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                             timeout=10, check=True)
    print(process.stdout)
    process = subprocess.run(['python', '-c', 'import coptpy'], text=True, stdout=subprocess.PIPE,
    ...
    early_failed = []
    to_run = []
    with open(args.input_file, 'r', encoding='utf-8', errors='ignore') as fd:
        for line in fd:
    ...
    if (args.question_field is not None) and (args.answer_field is not None):
        ...
        if args.majority_voting:
            ...
            for question, pred_answers in question2pred_answers.items():
                ...
                if args.verbose:
                    print(
                        f"gt_answer: {gt_answer}; pred_answers_t: {pred_answers_t}; mj_answer: {mj_answer}; is_mj_match: {is_mj_match}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="执行和评估生成的结果")
    parser.add_argument("--input_file", type=str, default="executed.jsonl", help="输入的JSONL文件，包含生成的结果")
    parser.add_argument("--output_file", type=str, default="execution_results.jsonl", help="输出的JSONL文件，包含执行结果和评估指标")
    parser.add_argument("--timeout", type=int, default=600, help="每个任务的超时时间（秒）")
    parser.add_argument("--max_workers", type=int, default=32, help="最大并行工作线程数")
    parser.add_argument("--verbose", action="store_true", help="启用详细输出模式")
    parser.add_argument("--majority_voting", action="store_true", help="启用多数投票机制")
    parser.add_argument("--question_field", type=str, default="question", help="输入文件中问题字段的名称")
    parser.add_argument("--answer_field", type=str, default="answer", help="输入文件中答案字段的名称")
    parser.add_argument("--numerical_err_tolerance", type=float, default=0.05, help="数值误差容差（相对误差）")
